...
function battery_widget:get_state()
    ...
    if pcall(function() state     = trim(readfile(dir.."/status"):lower())end) then
    ...
    ac_state  = readfile(pre.."/AC0/online")
...
function battery_widget:update()
    local percent, ac_state, time, state, is_charging, capacity, design 
= self:get_state()
    local prefix, text
